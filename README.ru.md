...
| **DeepSeek Harness** | ⚠️ нативный адаптер: runtime-проверен для конвейера инструментов, guard-слоя и `Stop`. Штатный hook-мост на `0.1.5-rc.2` измерен как **непригодный** | Cordis-плагин (`adapters/dsh/plugin.mjs`) на `ctx.tools.guard` + `tools/pre-execute`; см. [`docs/DSH.md`](docs/DSH.md) |
...
| **Codex, агенты с AGENTS.md** | 📄 только контракт | правило, которое агент обязан выполнить до объявления задачи сделанной |
...
Строка DeepSeek Harness — та, что менялась последней, и менялась она сначала в неприятную
сторону. Штатный hook-мост Claude Code регистрируется, срабатывает и не может запустить ни
одного хука: исполнитель в песочнице, которым он пользуется, читает `this.ctx.sandboxPolicy`,
а `this.ctx` разрешается в контекст **вызывающего**, которого у моста нет — поэтому каждый хук
возвращает пустое решение, и инструмент исполняется. Не запустившийся хук выглядит ровно как
гейт, решивший промолчать. Нативному адаптеру сервис `shell` не нужен, поэтому он этим не
затронут. Обе половины воспроизводятся командой `node scripts/jev-dsh-acceptance.mjs`.
