...
export async function verifyCompletion({
  ...
  const completion = decideCompletion({
    ...
    thresholds,
    judgeStatus: verdict.status ?? JUDGE_STATUS.ok,
 ...
 *
 * The status is mapped onto `JUDGE_STATUS`, which has exactly four members:
 * `ok`, `unavailable`, `abstained`, `notRequested`. There is no `error` member, and
 * reaching for one yields `undefined` — which `decideCompletion` then compares
 * against strings and silently fails to match. A judge that failed would have been
 * reported as a judge that answered. `unavailable` is the honest mapping for "we
 * could not get an answer".
 *
 * The transport is also wrapped, because `askJudge` lets a transport error escape
 * rather than reporting it. Left unwrapped, a refused connection would travel up
 * through the capability as an exception: the model would get a thrown error
 * instead of an envelope, and a gate that cannot answer would fail to say so. The
 * rule is that a gate which cannot answer must answer `unverified`, never crash and
 * never stay silent.
 *
...
export function judgeFromClient(options = {}) {
  return async ({ state, questions, model }) => {
    let response;
    try {
      response = await askJudge({ state, questions, model: model ?? undefined, ...options });
    } catch (error) {
      return {
        status: JUDGE_STATUS.unavailable,
        answers: {},
        answerErrors: [],
        reason: `the judge transport threw: ${error instanceof Error ? error.message : String(error)}`,
        usage: null,
        attempts: null,
      };
    }
    return {
      status: response?.status ?? JUDGE_STATUS.unavailable,
